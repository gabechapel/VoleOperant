...
// SECTION 1, First set the tone for time1 seconds
digitalWrite(startTone, HIGH);
delay(time1); //Wait 2 seconds
digitalWrite(startTone, LOW); // Turn off the tone

//SECTION 2, Wait time2 seconds or until there is a lever press to dispense the pellet
...
while (dispense == 0){
   ...
   if (PIState == 1 && PIState != lastState) { //If its unblocked and wasn't unblocked before, PELLET WAS TAKEN  
     ...
     delay(time3); //Keep the tone going for time3 seconds
